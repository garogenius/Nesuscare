...
def display_header():
    subprocess.run(["figlet", "-f", "slant", "AUREAT TOOL | by Garogenius", "|", 'lolcat'])
...
def main():
    ...
    parser.add_argument('--add', '-d', help="Add device to the monitoring list. Example: aureat --add -d <IP> -n 'name' --type 'device_type'", required=False)
    ...
    parser.add_argument('--report', help="Generate a report of correlated threats and save to a file.")
    ...
    parser.add_argument('--help', action='store_true', help="List all commands and how to use them.")
    ...
    if args.help:
        print("""
Aureat - Automated Threat Intelligence Aggregator by Garogenius

Usage:
  --add -d <IP> -n "name" --type "device_type"  Add a device with name and type to the monitoring list.
  --remove -D <IP>       Remove a device from the monitoring list.
  --drop -ML             Remove all devices from the monitoring list.
  --ML -l                List all devices in the monitoring list.
  --fetch                Fetch the latest threat data.
  --correlate            Correlate threats with the monitored devices.
  --report <title>       Generate and save a threat report.
  --alert <email>        Send email alerts for detected threats.
  --dg                   Detect dangerous threats and send an alert.
  --autocheck <email>    Enable auto-checking for threats and periodic alerts.
  --save <path>          Save the list of monitored devices.
  --version              Display the current tool version.
  -F <title> <content>   Send feedback or report an issue to the developer.
        """)
    elif args.version:
        print(VERSION)
        ...
        if args.correlate:
            correlated_threats = correlate_threats(threat_data)
            if args.report:
                generate_report(correlated_threats, args.report)
        if args.dg:
            dangerous_threats = detect_dangerous_threats(threat_data)
            if dangerous_threats:
                generate_report(dangerous_threats, "Dangerous Threats Report")
                send_email_alert("Dangerous Threats Detected", "Critical or malware threats have been detected.", args.alert)
        if args.alert:
